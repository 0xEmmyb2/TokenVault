...
import { handleTransactionError, ERC20_ABI } from "../lib/Utility";

const AGRITOKEN_ADDRESS = process.env.NEXT_PUBLIC_AGRITOKEN_ADDRESS || "";
const CURRENCY = process.env.NEXT_PUBLIC_CURRENCY || "ETH";
...
const PER_TOKEN_USD_PRICE = process.env.NEXT_PUBLIC_PER_TOKEN_USD_PRICE || "0";
...
interface TokenBalances {
  userTbcBalance: string;
  contractEthBalance: string | null;
...
interface Web3ContextType {
  ...
  reCall: number;
  globalLoad: boolean;
  buyToken: (ethAmount: string) => Promise<any>;
  updateTokenPrice: (newPrice: string) => Promise<any>;
  setSaleToken: (tokenAddress: string) => Promise<any>;
  withdrawAllTokens: () => Promise<any>;
  rescueTokens: (tokenAddress: string) => Promise<any>;
  formatAddress: (addr: string) => Promise<any>;
...
export const Web3Provider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [globalLoad, setGlobalLoad] = useState<boolean>(false);
  ...
  const buyToken = async (ethAmount: string): Promise<any> => {
    ...
    try {
      ...
      if (receipt.status === 1) {
        const tokenPrice = parseFloat(PER_TOKEN_USD_PRICE);
  ...
  const saveTransactionToLocalStorage = (txData: TransactionData): void => {
    try {
  ...
  const updateTokenPrice = async (newPrice: string): Promise<any | null> => {
    if (!contract || !address) return null;
  ...
  const setSaleToken = async (tokenAddress: string): Promise<any> => {
    if (!contract || !address) return null;
  ...
  const rescueTokens = async (tokenAddress: string): Promise<any> => {
    if (!contract || !address) return null;
  ...
  const formatTokenAmount = (amount: any, decimals = 18): string => {
    if (!amount) return "0";
